...
import { myDataUser } from "./Nilai";

const LIMIT = 4;

export default function Settings(props) {
  const data = myDataUser;
  const dataDepartment = myData;
      ...
      try {
        ...
        if (!query.empty) {
          const newUsers = await Promise.all(
            query.docs.map(async (users) => {
              return { id: users.id, ...users.data() };
            })
            );
            setUsers({
            user : newUsers.filter((a) => a.role === 'user'),
            admin : newUsers.filter((a) => a.role === 'admin')
          });
        }
      } catch (error) {
  ...
  const handleDelete = async (data, versi) => {
    // console.log(data.id);
    const api = versi === "User" || versi === "Admin" ? "user-delete" : "";
    try {
      const deteleData = firefunctions.httpsCallable(api);
      const response = await deteleData({ user_id: data?.id });
  ...
  return (
    <div
      className={`flex justify-between flex-row duration-300 pb-12 hide-scroll-bar `}
    >
      {/* start body */}
      <div className={`w-full duration-300 `}>
        <div className="flex justify-between mb-12">
          <div className={`flex mr-12 duration-200 w-[80%]`}>
            <input
              className="px-12 py-6 w-full text-[16px] text-trans3 border-[1px] border-trans3 rounded-md border-opacity-30 focus:ring-trans1 focus:ring-1 focus:outline-none focus:border-trans1 focus:border-opacity-50 duration-200"
        ...
        <div className="grid grid-cols-2">
          <div className="w-[96%]">
            ...
            <span className="text-neutral-500 text-[14px]">
              Admin bisa menambah dan menghapus users dan memberikan nilai
              terhadap user
            </span>
          </div>
          <div>
            <table className="table-auto w-full rounded">
              <thead>
                <tr className="text-left bg-c2">
                  <th className="pl-8 py-6 text-[14px] font-bold">Admin</th>
                  <th className="pl-8 py-6 text-[14px] font-bold">
                    Ditambahkan
                  </th>
              ...
              <tbody>
                {users?.admin?.map((data, index) => {
                  return (
                    <tr
                      key={index}
                      className={`border-b-2 border-trans1 border-opacity-10 items-center justify-center`}
                    >
                      <td className="pl-6 text-[18px] font-semibold]">
                        <div className="flex items-center">
                          ...
                          <div className="max-w-[120px]">
                            <h1 className="text-[16px] font-bold capitalize truncate">
                              {data.username}
                            </h1>
                      ...
                      <td
                        className={`pl-8 pt-8 pb-6 text-[14px] font-semibold text-neutral-800`}
                      >
                        Feb 22 - Teddi
                      </td>
                      <td className={`pl-8 pt-8 pb-6 text-[14px]`}>
                        <div className="flex">
                          <div
                            className="p-4 cursor-pointer mr-2"
                            onClick={() => handleDelete(data, "Admin")}
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              className="h-8 w-8"
        ...
        <div className="grid grid-cols-2 my-12">
          <div className="w-[96%]">
            ...
            >
              <h1 className="font-bold text-[20px] mr-4">Department</h1>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-8 w-8 stroke-2"
                color="#242424"
        ...
        <div className="grid grid-cols-2">
          <div className="w-[96%]">
            ...
            <span className="text-neutral-500 text-[14px]">
              Hukuman akan diberikan oleh department kebersihan kepada
              department yang memiliki nilai paling rendah.
            </span>
          </div>
          <div>
            <table className="table-auto w-full rounded">
              <thead>
                <tr className="text-left bg-c2">
                  ...
                  <th className="px-8 py-6 text-[14px] font-bold grid-cols-2 grid">
                    Action
                  </th>
                </tr>
              </thead>
                      ...
                      <td className="py-4 text-[18px] font-semibold">
                        <div className="pl-6 w-[90%]">
                          <div className="flex mt-4">
                            ...
                            <span className="text-[12px] text-neutral-500">
                              Hukuman diberikan kepada department yang memiliki
                              nilai paling rendah.
                            </span>
                          </div>
                        </div>
